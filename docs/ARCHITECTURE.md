...
- **Numeric extraction is regex + a curated word-number list (one–ninety), not a general number parser.** "A couple hundred" or "dozens of" won't be caught. The count-context word list (`people`, `killed`, `percent`, ...) is hand-curated for hard-news vocabulary.
...
- Run everything with `mvn test`; there is no hidden integration-only path — the scoring logic that ships is the scoring logic that's tested.
